...
public function indexAction() {
  ...
  $sqlUsers = "SELECT * FROM `admin_users` WHERE `utype` != 'super_admin' ";
...
public function adduserAction() {
    ...
    if($validationErrors) {
      ...
      if(isset($post['id_to_edit']) && $post['id_to_edit']) {
        ...
        if($modelUser->updateData($updateUser,$post['id_to_edit'])) {
          ...
          $this->modelLog->insertLogEntry($logText,$this->user['id_admin_user'],serialize($post),$post['id_to_edit'],'addadminuser','id_configuration');
  ...
  if(isset($user) && $user) $this->view->edit = $user;
	...
	public function deleteuserAction(){
		...
		if(isset($get['del']) && $get['del']){
			if($modelUser->deleteData($get['del']))	{
				// insert log entry
				$logText = 'User '.$this->user['name'].' ( User# '.$this->user['id_user'].') deleted a user user# '.$get['del'];
				$this->modelLog->insertLogEntry($logText,$this->user['id_user'],serialize($get),$get['del'],'deleteuser','','Backoffice activity','');
	...
	public function changepasswordAction() {
		...
		$breadcrumbs[] = array('link' => 'index', 'label' => 'Dashboard', 'icon' => 'fa fa-dashboard');
		$breadcrumbs[] = array('link'=>'users/listuser','label'=>'Users','icon'=>'fa fa-user');
		$breadcrumbs[] = array('link'=>'','label'=>'Users','icon'=>'');
		$this->view->breadcrumbs = $breadcrumbs;
		...
		if(isset($get['id_user']) && $get['id_user']) {
			...
			if(($user_type == 3) && ($this->user['utype'] > 3)){
				$urlError = array('controller'=>'index', 'action'=>'accesserror');
				$this->_helper->redirector->gotoRoute($urlError);
			}
					...
					if($validationErrors) {
						...
						if(isset($idToEdit) && $idToEdit) {
							...
							if($modelUser->updateData($updateUser,$idToEdit)) {
								// insert log entry
								$logText = 'User '.$this->user['name'].' ( User# '.$this->user['id_user'].') change password of user user# '.$idToEdit;
								$this->modelLog->insertLogEntry($logText,$this->user['id_user'],serialize($post),$idToEdit,'changepassword','','Backoffice activity','');
								$message = $this->_composeMessageHtml('Password updated successfully','success');
							} else {
								$frmData = $post;
								$message = $this->_composeMessageHtml('Something went wrong while updating the password. Please try again later.','error');
							}
						}
					}
				}
			...
			if(isset($this->user['id_user']) && $this->user['id_user']) {
				...
				if($request->isPost()) {
					...
					$validationErrors = $this->_validateChangePassword($post);
					//echo '<pre>';	print_r($validationErrors);echo '</pre>';
					if($validationErrors) {
						$message = $this->_composeMessageHtml('You have some validation errors. Please fix them to continue.','error');
					} else {
						if(isset($idToEdit) && $idToEdit) {
	...
	public function _validateUserCreation($post,$idToEdit = '') {
		...
		if($idToEdit ==''){
			...
			else if(!$modelUser->isUnique('email',$post['email'],' AND `id_user` != '.$userToEdit['id_user'])) $errors['email'] = 'This email id has been already taken.';
			if(!isset($post['utype']) || !trim($post['utype'])) $errors['utype'] = 'Please select a user type';
			if(!isset($post['status']) || ($post['status'] == '')) $errors['status'] = 'Please select a status';
		}
