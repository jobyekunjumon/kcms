...
class User_MediaController extends Zend_Controller_Action {
  ...
  public function asyncGetMediaLibraryAction() {
    ...
    if(isset($post['id_user']) && $post['id_user']) {
      $searchCondition .= $conj.' `id_user` = '.$post['id_user'];
      $conj = ' AND ';
    } else {
      $searchCondition .= $conj.' `id_user` = 0';
      $conj = ' AND ';
    }
    ...
    if($medias) {
      ...
      $out = '<h2>No media entries found. Please check stock images or upload images.</h2>';
    }
  ...
  public function asyncUploadMediaAction() {
    $layout = $this->_helper->layout();
    $layout->disableLayout();
    $this->_helper->viewRenderer->setNoRender();
    // insert media
    ...
    if(isset($_FILES['file']) && $_FILES['file']) {
      ...
      if (!in_array($extension, $allowedExts)) {
        ...
        if(move_uploaded_file($file['tmp_name'],$filename)) {
          ...
          'uploaded_by' => $this->user['id_user'],'id_user' => $this->user['id_user'],'file_name' => $fileTitle,
