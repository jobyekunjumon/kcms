...
class AuthController extends Zend_Controller_Action
{
  ...
  public function indexAction()
  {
    ...
    if($request->isPost()) {
      ...
      if($validationErrors) {
        ...
        //$idUser = $this->_processLogin($email,$passwordHash);
        if($idUser = $this->_processLogin($email,$passwordHash)) {
          ...
          $urlUserDash = array('controller'=>'user', 'action'=>'index');
  ...
  protected function _processLogin($email,$password)
  {
    ...
    protected function _getAuthAdapter()
    {
        ...
        $authAdapter->getDbSelect()->where('user_status = 1');
        return $authAdapter;
    }
